...
During Modern Warfare II, I spent the majority of my time in Search and Destroy attempting to hit cross-map throwing knives. After I hit Embassy, the last 6v6 map I needed, I had amassed 91 cross-maps in total. After omitting Shipment, I tried to pull out a few interesting details to share with the community. This dataset will also be available on GitHub if anyone wants to use it in the future.
...
Crown Raceway has a relatively large discrepancy in spawn safety, with defense holding the distinct advantage. The left side of the tall, central building naturally protects players until they want to run up the stairs to mid-street --- the most logical area for hitting cross-maps from offense (Figure 2). I hit several here which I'd consider fairly lucky. It is a relatively high traffic area; however, the variability in the throw combined with having to clear the concrete barrier, hitting a narrow kill zone, and underutilisation of the upper platform (which knives tend to bounce up on) make this more challenging than it looks. Note that hitting AFK players directly at spawn is also possible, but I'd argue it requires more luck.
...
![Figure 6. The A corridor and the window a throwing knife needs to go through to be successful. Note that aiming towards the exit (i.e., further left) is also viable.](images/taraq2-01.png){fig-align="center"}

Although the defensive throw is pretty blind (due to blue), the offense spawn is more forgiving and does us a subtle, but key favour. The characters tend to spawn in looking more left (towards A), which perhaps leads to more A rushes; it may also have no effect on player decision-making (Figure 7). Regardless, the offense path to A is really the only viable cross-map option (as the B rush is well guarded) --- lucky us. Left of spawn is pretty open and the perpendicular runway offers lots of lead time if you can avoid the retaining wall and the corner building on the right (Figure 8). AFK players are also in play and a consistent point and release throw with strong arm makes this easier than expected.
...
Zarqwa Hydroelectric is simply a cross-mapping paradise; however, there are some key factors as to why kills on offense seem to be harder to come by than its counterpart. Looking at the offense knife kill zone, the long runway to mid is relatively good from a cross-map perspective, but the players are protected by the generator building and its electric towers and are actually at a slightly lower elevation due to the carved out path on the right. Moreover, situated right in front of this path are various liquid containers and a power pole, which may play a sneaky role in reducing success rate. I figure a few lives were saved due to their presence (i.e., deflections). Ultimately, knives have to cross the map, clear the building, and get down fast which allows many players to be safe by that time (if they tac-sprint at the start of the round).

![Figure X. The area of interest for the cross-map thrown from offense.](images/hydroelectric2.png){fig-align="center"}

Conversely, tossing knives from the defense spawn is a dream setup. Player spawns in the kill zone? Check. Point and release strong arm throw? Check. Long, straight runway with no barriers and bounce opportunities? Check. Perfect bomb placement that entices players to funnel down the middle lane? Check. Thus, I started expecting to hit one from this side every game as it is a textbook example of what to look for when cross-mapping. Although this is a small sample size, offense on Zarqwa Hydroelectric may very well be the most dangerous Search and Destroy spawn in the game.

**Conclusion:** Defense everytime.

![Figure X. The nightmare scenario for players spawning and subsequently running middle on offense.](images/hydroelectric1.png){fig-align="center"}

## Annual data

Lastly, I thought it was interesting to look at the temporal aspect of the data (i.e., when did these kills happen over the course of the year?). Looking at Figure 11, it's pretty evident that these occurred in three distinct time frames:
...
#| fig-align: center
#| cap-location: bottom
#| fig-cap: Figure 11. Daily cross-map kills during the Modern Warfare II game cycle. Start of years are indicated by blue dotted lines.
